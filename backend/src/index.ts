...
// Request log. Method, path, status, duration and client IP only — never the
// body, which carries passwords and patient data. Off by default in packaged
// builds; set REQUEST_LOG=1 (or run in dev) to see traffic from other devices.
const requestLogEnabled =
  process.env.REQUEST_LOG === '1' || process.env.NODE_ENV !== 'production';
if (requestLogEnabled) {
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      console.log(
        `${req.method} ${req.originalUrl} ${res.statusCode} ${Date.now() - startedAt}ms ${ip}`
      );
    });
    next();
  });
